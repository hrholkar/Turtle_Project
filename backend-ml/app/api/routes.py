...
async def predict_turtle(
    ...
    log.info(f"[API] /predict called. Image side: {side_upper}, size: {len(image_bytes)} bytes")
    ...
    log.info(f"[API] /predict success. Matched: {result['matched']}")
    return result
...
async def register_new_turtle(
    ...
    image_bytes = await image.read()
    side_upper  = image_side.upper().strip()
    log.info(f"[API] /register_new_turtle called. ID: {identity}, Species: {species}, Side: {side_upper}")
    ...
    log.info(f"[API] /register_new_turtle success for {identity}.")
