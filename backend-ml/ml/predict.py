...
def _normalise_species(s: str) -> str:
    ...
    # Canonical mapping between frontend keys and ML classifier labels
    mapping = {
        "green": "green sea turtle",
        "green turtle": "green sea turtle",
        "olive": "olive ridley",
        "olive_ridley": "olive ridley",
        "kemp_ridley": "kemps ridley",
        "flatback": "flatback",
...
def search_faiss_filtered(
    ...
    for dist, idx in zip(distances[0], indices[0]):
        ...
        # ── Species filter ────────────────────────────────────────────────────
        # Removed species filter: It prevented matching exact images if the classifier
        # prediction disagreed with the user's manual label during registration.
...
def predict(
    ...
    if raw_matches and top_similarity >= threshold:
        log.info(
            "Match found! (top_similarity=%.3f >= threshold=%.3f) → IDENTITY: %s",
            top_similarity,
            threshold,
            raw_matches[0]["identity"],
        )
        return {
            "matched":            True,
