...
const mlClient = axios.create({
  ...
  timeout: 180_000,  // 180 s — embedding extraction / cold start can take time
});
...
export class MLService {
  ...
  static async predict(
    ...
    console.log(`[MLService] Preparing to POST /predict ... imagePath: ${imagePath}, imageSide: ${imageSide}`);
    ...
    try {
      ...
      console.log(`[MLService] /predict Success. Matched: ${response.data.matched}`);
  ...
  static async registerNewTurtle(params: {
    ...
    console.log(`[MLService] Preparing to POST /register_new_turtle ... identity: ${params.identity}`);
    const form = new FormData();
    ...
    try {
      ...
      console.log(`[MLService] /register_new_turtle Success for identity: ${params.identity}`);
      return response.data;
    } catch (err: any) {
      console.error(`[MLService] /register_new_turtle Failed:`, err.message);
      throw err;
    }
  }
