...
export class SightingController {
  ...
  static async createManual(req: Request, res: Response, next: NextFunction) {
    try {
      // Allow imagePath to come from a previous upload (temporary URL) or a new file upload
      let imagePath = req.body.imageUrl;
      if (req.file) {
        imagePath = req.file.path;
      }
      
      if (!imagePath) throw createError('No image provided', 400);

      const params = {
        turtleId: req.body.turtleId,
        imagePath,
        location: req.body.location,
        latitude: req.body.latitude ? parseFloat(req.body.latitude) : undefined,
        longitude: req.body.longitude ? parseFloat(req.body.longitude) : undefined,
        sightingDate: req.body.sightingDate ? new Date(req.body.sightingDate) : new Date(),
        notes: req.body.notes,
        confidenceScore: req.body.confidenceScore ? parseFloat(req.body.confidenceScore) : undefined,
      };

      const result = await SightingService.createManual(params);
